...
```{r}
joined_returns$rstd <- (joined_returns$r - mean(joined_returns$r))/sd(joined_returns$r)
...
- ARCH(1), GARCH(1), GARCH(2) are insignificant. 
- ACF of standardized residuals shows no autocorrelation.
...
- ARCH(1) and GARCH lags are insignificant, except for GARCH(5). 
- ACF of standardized residuals shows no autocorrelation.
...
```{r}
plot_var = function(dates, returns, vars){
  plot(dates, returns, 
...
```{r}
chosen_garch <- fit_garch(insample_r$r, 0, 0, 3, 3)
```
...
```{r}
threshold = 0.01
var_q <- quantile(insample_r$rstd, threshold)
var_q
```
...
```{r}
vars <- var_q * chosen_garch@fit$sigma
```
```{r}
plot_var(insample_r$Date, insample_r$r, vars)
```

In how many days do we breach the VaR threshold of `r threshold`?

```{r}
var_losses(insample_r$r, vars)
```
A little worse than expected. Let's test with level 5%.

```{r}
vars <- quantile(insample_r$rstd, 0.05) * chosen_garch@fit$sigma
plot_var(insample_r$Date, insample_r$r, vars)
var_losses(insample_r$r, vars)
```
Level 5% is in expected range.
