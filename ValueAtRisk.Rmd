...
```{r}
chosen_garch <- fit_garch(insample_r$r, 0, 0, 2, 1)
```
...
```
Worse than expected. Let's test with level 5%.

```{r}
vars <- get_var_quantile(insample_r$r, 0.05) * chosen_garch@fit$sigma
plot_var(insample_r$Date, insample_r$r, vars)
var_losses(insample_r$r, vars)
```
