...
We should consider empty values for weekends and bank holidays. Instruments are obviously present in the portfolio during non-trading days, the difference is that their quotes are frozen, which is the same as their daily return on non-trading days is zero. Hence, we are replacing empty return values for non-trading days with zeros.


```{r}
joined_returns = joined_returns %>% slice(-1) %>% replace(is.na(.), 0)
```

Now let's calculate the log return of portfolio ($w_i=w=1/N$, $R$ stands for arithmetic return, $r$ - for logarithmic return).

$$
ln \dfrac{\Pi_{t+1}}{\Pi_{t}} = ln\dfrac{\sum_{i=1}^{N}S_{i,t+1}}{\Pi_t}=ln\dfrac{1/N\,\sum_{i=1}^{N}\Pi_{t}R_{i,t}}{\Pi_t} = ln \dfrac{1}{N}\sum_{i=1}^{N}R_{i,t} = ln \dfrac{1}{N}\sum_{i=1}^{N}e^{r_{i,t}}
$$
...
joined_returns$Portfolio = log(1/length(tickers) * ret_sum)
```

### Plots of the log returns.
```{r}
plot_quotes(joined_returns, "Portfolio", sprintf("%s differenced log returns", "Portfolio"))
```
...
```{r}
acf(joined_returns$Portfolio, lag.max = 36, na.action = na.pass,
   ylim = c(-0.2, 0.2), col = "darkblue", lwd = 4,
   main = "ACF of portfolio returns")
acf(joined_returns$Portfolio^2, lag.max = 36, na.action = na.pass,
...
- There are a couple of lags for plain log returns, but they are not very pronounced and on the verge of insignificance.
