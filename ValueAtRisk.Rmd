...
```{r}
arma00egarch33 <- fit_garch(insample_r$r, 0, 0, 1, 3,model_type = "eGARCH")
arma00egarch33
```
...
#### ARMA(0, 0)- TGARCH(3, 3)
```{r}
arma00tgarch33 <- fit_garch(insample_r$r, 1, 1, 3, 3,model_type = "fGARCH", submodel = "TGARCH")
arma00tgarch33
```
...
- AR(1), MA(1), ARCH(2) and GARCH (1) terms are significant

- The significant eta11 term indicates asymmetric effects in the TGARCH model

- Ljung-Box and LM tests show no serial autocorrelation, except for Lag(5) for LB on standardised residulas
...
- AR(5), MA(5) are highly significant
- AR(1), MA(1), MA(2), GARCH(1) terms as well as asymmetry terms for TGARCH(1,1) are significant
-Ljung-Box and LM test show no serial autocorrelation between the residuals
