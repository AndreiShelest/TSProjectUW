...
```{r}
joined_returns <- joined_returns %>% replace(is.na(.), 0) %>% column_to_rownames(., var = "Date")
```
...
```
```{r}
arma00garch11 <- fit_garch(0, 0, 1, 1)
arma00garch11
```
...
```{r}
arma00garch21 <- fit_garch(0, 0, 2, 1)
arma00garch21
```
...
```{r}
arma00garch12 <- fit_garch(0, 0, 1, 2)
arma00garch12
```
```{r}
...
```{r}
arma00garch22 <- fit_garch(0, 0, 2, 2)
arma00garch22
```
```{r}
...
- ARCH(1), ARCH(2) and GARCH(2) parameters are not significant. 
- ACF of standardized residuals shows almost no autocorrelation.
- ACF of squared standardized residuals shows significant third lag.
- Both Ljung-Box test for standardized squared residuals and LM tests do not allow us to reject null-hypothesis of no autocorrelation of said residuals.
...
```{r}
arma00garch33 <- fit_garch(0, 0, 3, 3)
arma00garch33
```
...
- ARCH(1) and GARCH(2) parameters are not significant. 
- ACF of standardized residuals shows almost no autocorrelation.
...
### ARMA(7,0)-GARCH(2,1)
```{r}
arma70garch21 <- fit_garch(7, 0, 2, 1)
arma70garch21
```
...
### ARMA(7,7)-GARCH(2,1)
```{r}
arma77garch21 <- fit_garch(7, 7, 2, 1)
arma77garch21
```
```{r}
...
- AR(2), AR(4), AR(7), MA(2), MA(4), MA(7) lags are insignificant, as well as ARCH(2). 
...
### ARMA(5,5)-GARCH(2,1)
```{r}
arma55garch21 <- fit_garch(5, 5, 2, 1)
arma55garch21
```
```{r}
...
### ARMA(5,5)-GARCH(2,2)
```{r}
arma55garch22 <- fit_garch(5, 5, 2, 2)
arma55garch22
```
```{r}
...
- Again, only ARCH(2) is insignificant. 
...
### ARMA(5,5)-GARCH(3,3)
```{r}
arma55garch33 <- fit_garch(5, 5, 3, 3)
arma55garch33
```
```{r}
...
- More lags are insignificant: AR(2), AR(3), MA(2), MA(3), ARCH(1), ARCH(2), GARCH(1), GARCH(2). 
- ACF of standardized residuals showed significant 9th lag.
- ACF of squared standardized residuals shows significant third lag and 14th.
...
### ARMA(5,5)-GARCH(3,5)
```{r}
arma55garch35 <- fit_garch(5, 5, 3, 5)
arma55garch35
```
```{r}
...
- All GARCH lags are insignificant, except for GARCH(5). 
- ACF of standardized residuals shows no autocorrelation.
- ACF of squared standardized residuals also shows no autocorrelation (significance of 3rd lag is marginal).
- Both Ljung-Box test for standardized squared residuals and LM tests do not allow us to reject null-hypothesis of no autocorrelation of said residuals. 
