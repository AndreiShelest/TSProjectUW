...
for(ticker in tickers){
  ...
  if(is.null(joined_returns)) {
    joined_returns = ticker_df %>% 
      select(Date, R) %>% 
      rename({{ticker}}:= R)
  }
  else {
    joined_returns = full_join(joined_returns, 
                               ticker_df %>% 
                               select(Date, R) %>%
                               rename({{ticker}}:= R), by="Date")
  }
...
plot_quotes(tickers_map[[tickers[2]]], "R", sprintf("%s differenced log returns", tickers[2]))
```
...
plot_quotes(tickers_map[[tickers[3]]], "R", sprintf("%s differenced log returns", tickers[3]))
```
...
plot_quotes(tickers_map[[tickers[4]]], "R", sprintf("%s differenced log returns", tickers[4]))
```
...
plot_quotes(tickers_map[[tickers[5]]], "R", sprintf("%s differenced log returns", tickers[5]))
```
