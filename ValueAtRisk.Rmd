...
- The significant omega, alpha1, beta1, and gamma1 terms indicate the presence of volatility clustering, persistence in volatility, and leverage effects

- Tests suggest that the model adequately captures the volatility dynamics, with no significant remaining autocorrelation or ARCH effects, although there are some indications of potential remaining ARCH effects in the weighted ARCH LM tests
...
Based on the criteria above, ARMA(5,5)-TGARCH(1,2) has the lowest AIC, Shibata, and Hannan-Quinn values, and is 4th in BIC. Therefore, **ARMA(5,5)-TGARCH(1,2)** is the best choice among the tested models as it seems to provide the best balance between model fit and complexity.

```{r}
chosen_garch = function(returns){
  return(fit_garch(insample_r$r, 5, 5, 1, 2,model_type = "fGARCH", submodel = "TGARCH"))
}
```
...
- Significant terms: AR (1, 3, 4, and 5), MA (1, 2, 3, 4, and 5)

- omega - significant - constant term in the variance equation

- alpha1 - significant- represents ARCH effect

- beta2 - significant - indicates a high persistence of volatility

- eta11 - significant - represents the levarage effect


### Nyblom stability test
Joint statistic below critical values = 2.6991 is below the critical value = 4.3 at 1% confidence interval. Hence we don't reject the null hypothesis. Individual statistics are also within range.
...
Values suggest a good fit with significant p-values for different groups.

### News Impact Curve

```{r}
plot_news_impact_curve(arma55tgarch12)
```
...
```{r}
plot_r_std_density(arma55tgarch12)
```
```{r}
std_residuals = (arma55tgarch12@fit$residuals 
                 - mean(arma55tgarch12@fit$residuals)) / arma55tgarch12@fit$sigma
